...
p_sc <- ng_pgs %>% 
    ...
    filter(name %in% str_c('Factor', 1:3)) %>% 
    inner_join(ng_res) %>% 
    mutate(name = case_when(name == 'Factor1' ~ 'Core language (F1)',
                            name == 'Factor2' ~ 'Receptive language (F2)',
                            name == 'Factor3' ~ 'Nonverbal IQ (F3)')) %>% 
    mutate(name = factor(name, levels = c('Core language (F1)', 'Receptive language (F2)', 'Nonverbal IQ (F3)'))) %>%
    mutate(lab = str_c('r = ', round(estimate, digits = 2), ', p = ', formatC(p.value, digits = 2))) %>% 
    mutate(sig = ifelse(p.value < 0.05, TRUE, FALSE)) %>%
    filter(name != 'Receptive language (F2)') %>%
    ggplot(aes(y = pgs_resid, x = value)) +
    geom_point(aes(color = sig), size = 0.6) +
    geom_smooth(method = 'lm', size = 1.5, aes(color = sig)) +
    facet_wrap(~ name) +
    geom_text(aes(x = 0, y = 2.9, label = lab), check_overlap = TRUE, size = 4) +
    ylab('Prenatal scQTL HAQER CP-PGS') +
    xlab('Factor score') +
...
ggsave(p_ht,
       filename = 'manuscript/figures/EpiSLI_scQTL_HAQER_CP-PGS_heatmap.png',
       device = 'png', dpi = 300, bg = 'white',
       units = 'in', width = 9, height = 7)

ggsave(p_sc,
       filename = 'manuscript/figures/EpiSLI_scQTL_HAQER_CP-PGS_scatterplot.png',
       device = 'png', dpi = 300, bg = 'white',
       units = 'in', width = 7, height = 4)

ng_res %>% 
    mutate(name = str_remove_all(name, 'actor')) %>% 
    rename(factor = name, cp_pgs.HAQER_scQTL = haqer_pgs) %>% 
    filter(cp_pgs.HAQER_scQTL == 'all') %>% 
    mutate(cp_pgs.HAQER_scQTL == 'all_Jerber_scQTLs') %>% 
    write_csv('manuscript/supplemental_materials/stats/EpiSLI_factor_HAQER_scQTL_CP-PGS_stats.csv')

## save figure objects
p_ht %>% 
    write_rds('manuscript/figures/R_plot_objects/HAQER_scQTL_EpiSLI_validation_heatmap.rds')
p_sc %>% 
    write_rds('manuscript/figures/R_plot_objects/HAQER_scQTL_EpiSLI_validation_F1_F3_scatterplot.rds')
