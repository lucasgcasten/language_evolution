...
haqer <- read_tsv('/Dedicated/jmichaelson-wdata/lcasten/tools/ref_data/human_accelerated_regions/HAQERs/Mangan-Cell2022/other_regions/har.hg38.bed', col_names = FALSE)
...
haqer_chr <- haqer %>% 
    ...
    arrange(X2, X3) %>% 
    mutate(X2 = X2 - 10000, 
            X3 = X3 + 10000) ## 10Kb flank around ROI
...
message('There are ', sum(haqer_filter), ' variants within 10Kb of ROIs on this chromosome')
...
message('The genotype table looks like this:')
print(gd[1:5,1:3])

## print summary stats about HAQER variants
message('There are ', length(var_id), ' total variants on chromosome ', chr)
message('There are ', sum(haqer_filter), ' variants on this chromosome (max MAF = ', round(max(df$af[df$haqer == TRUE] * 100), digits = 3), '%)')
...
wd <- df_haq %>% 
    inner_join(df_af) %>%
    left_join(rename(anc_alleles, position = pos)) %>%
    inner_join(gd_t) %>% 
    filter(af > 0 & af < 1) %>%
...
## save to file
outf <- str_c('/Dedicated/jmichaelson-wdata/lcasten/sli_wgs/HAQER_variant_associations/data/SPARK_WGS/chr', chr, '_HAR_10Kb_flank.hg38.rds')
wd %>% 
    write_rds(outf)

############
## compute summary scores for reversion counts
tmp_brd <- data.frame(sample = colnames(wd)[str_detect(colnames(wd), 'SP')]) %>% 
        as_tibble()
...
tmp_rare <- wd  %>% 
    mutate(max_gnomAD_af = ifelse(is.na(max_gnomAD_af), 0, max_gnomAD_af),
           thousand_genomes = ifelse(is.na(thousand_genomes), 0, thousand_genomes)) %>%
    filter(af < 0.01 & max_gnomAD_af < 0.01 & thousand_genomes < 0.01 & alt == HCA_allele) %>% 
    select(matches('SP'))
tmp_brd$haqer_rare_variant_reversion_count = colSums(tmp_rare)

tmp_brd %>% 
    write_csv(str_c('/Dedicated/jmichaelson-wdata/lcasten/sli_wgs/HAQER_variant_associations/data/SPARK_WGS/chr', chr, '_HAR_10Kb_flank.hg38.reversion_count_summary.csv'))
