...
################################ 
## cog + non-cog skills
sumstats_files = list.files(path = '/Dedicated/jmichaelson-sdata/gwas_summary_stats/cog_nc/CogNonCog-Malanchini-NatHumBeh', 
...
ss_map = data.frame(filepath = sumstats_files, 
  ...
  mutate(pheno = str_replace_all(pheno, pattern = '_ext.txt.gz', replacement = 'ogntive_skills'))
...
for (i in 1:nrow(ss_map)) {
  ...
  if (! pheno %in% list.dirs(path = str_sub(my_wd, start = 1, end = -2), recursive = F, full.names = F)) {
    ...
    final_sumstats2<-subset(final_sumstats, final_sumstats$MAF <= .4 & final_sumstats$MAF >= .1)
    #calculate expected sample size (N_hat)
    N_hat<-mean(1/((2*final_sumstats2$MAF*(1-final_sumstats2$MAF))*final_sumstats2$SE^2))

    ##
    final_sumstats$Neff = 1173569
...
pheno = 'height_Yengo'
message('Calculating PGS for: ', pheno)
ss <- read_delim('/Dedicated/jmichaelson-sdata/gwas_summary_stats/anthropomorphic/height/GIANT_HEIGHT_YENGO_2022_GWAS_SUMMARY_STATS_EUR.gz', delim = '\t')
...
message('before GWAS sumstats QC there were ', nrow(ss), ' variants')
final_sumstats <- ss %>%
  rename(A1 = EFFECT_ALLELE, A2 = OTHER_ALLELE) %>%
  filter(nchar(A1) == 1 & nchar(A2) == 1) %>% 
  filter(A1 %in% c('A', 'C', 'T', 'G') & A2 %in% c('A', 'C', 'T', 'G'))
